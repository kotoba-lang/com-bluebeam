(ns bluebeam.main
  "Kotodama WASM entrypoint for the Bluebeam clean-room actor (L4) — Clojure port.
  ...
  facts (`bluebeam.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bluebeam")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bluebeam.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bluebeam-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bluebeam-compat/ready)
